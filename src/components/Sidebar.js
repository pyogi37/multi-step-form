...
const SideBar = () => {
  return (
    <Box
      className="sidebar"
      bg={`url("data:image/svg+xml,${encodeURIComponent(sidebarDesktop)}")`}
      ...
      p={2}
    >
      <Steps stepNumber={1} stepTitle="YOUR INFO" />
      <Steps stepNumber={2} stepTitle="SELECT PLAN" />
      <Steps stepNumber={3} stepTitle="ADD-ONS" />
      <Steps stepNumber={4} stepTitle="SUMMARY" />
    </Box>
  );
};
